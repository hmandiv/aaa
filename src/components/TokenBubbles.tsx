...
const TokenBubbles = ({
  ...
  useEffect(() => {
    ...
    if (initialTokens && initialTokens.length > 0) {
      ...
      if (
        "priceChange24H" in initialTokens[0] &&
        initialTokens[0].priceChange24H !== undefined
      ) {
        setIsLoading(false);
  ...
  const processBubbleTokens = (tokens: any) => {
    ...
    const tokensWithDefaults = tokens.map(
      (token: {
        priceChange24H: any;
        fullTVL: any;
        totalTVL: any;
    ...
    const sortedTokens = [...tokensWithDefaults].sort(
    ...
    if (selectedTokenIds.length === 0) {
      const initialSelected = sortedTokens
        .slice(0, 25)
        .map((token) => token.assetID);
      setSelectedTokenIds(initialSelected);
      setBubbleTokens(
        sortedTokens.filter((token) => initialSelected.includes(token.assetID))
      );
    } else {
      // Filter by selected tokens
      setBubbleTokens(
        sortedTokens.filter((token) => selectedTokenIds.includes(token.assetID))
      );
    }
  };
  ...
  const fetchPriceDataForTokens = async (tokens: any[]) => {
    ...
    try {
      ...
      const tokenPromises = tokens.map(
        async (token: { assetID: any; name: any }) => {
          try {
            // Fetch price change data
            const priceChangeResponse = await fetch(
              `https://api.vestigelabs.org/asset/${token.assetID}/prices/simple/${priceChangeInterval}`
            );
            ...
            if (priceChangeData && priceChangeData.length > 0) {
              ...
              const endPrice =
                priceChangeData[priceChangeData.length - 1]?.price || 0;
              priceChange =
                startPrice > 0
            ...
            console.error(
              `Error fetching price data for token ${token.name}:`,
              error
            );
            return {
              ...token,
      ...
      const tokenPromises = baseTokens.map(async (token) => {
        try {
          ...
          const tvlResponse = await fetch(
            `https://api.vestigelabs.org/asset/${token.assetID}/tvl/simple/7D?currency=USD`
          );
          ...
          const priceResponse = await fetch(
            `https://api.vestigelabs.org/asset/${token.assetID}/price?currency=usd`
          );
          ...
          const holdersResponse = await fetch(
            `https://api.vestigelabs.org/asset/${token.assetID}/holders?limit=10000000`
          );
  ...
  useEffect(() => {
    if (allTokens.length > 0 && selectedTokenIds.length > 0) {
      setBubbleTokens(
        allTokens.filter((token) => selectedTokenIds.includes(token.assetID))
      );
  ...
  return (
    <div className={styles.premiumContainer}>
          ...
          <div className={styles.intervalOptions}>
            <button
              className={`${styles.intervalButton} ${
                priceChangeInterval === "1H" ? styles.activeInterval : ""
              }`}
            ...
            <button
              className={`${styles.intervalButton} ${
                priceChangeInterval === "1D" ? styles.activeInterval : ""
              }`}
            ...
            <button
              className={`${styles.intervalButton} ${
                priceChangeInterval === "7D" ? styles.activeInterval : ""
              }`}
              ...
              <div className={styles.filterButtons}>
                <button
                  className={`${styles.filterButton} ${
                    filterGainers === true ? styles.activeFilter : ""
                  }`}
                  onClick={() =>
                    setFilterGainers(filterGainers === true ? null : true)
                  }
                >
                  Gainers
                </button>
                <button
                  className={`${styles.filterButton} ${
                    filterGainers === false ? styles.activeFilter : ""
                  }`}
                  onClick={() =>
                    setFilterGainers(filterGainers === false ? null : false)
                  }
                >
                  Losers
                </button>
              ...
              {filteredTokens.map((token) => (
                <div
                  ...
                  className={`${styles.tokenListItem} ${
                    selectedTokenIds.includes(token.assetID)
        ...
        <div className={styles.legendContainer}>
          ...
          <div className={styles.legendItem}>
            <div
              ...
              style={{ backgroundColor: "rgba(0, 180, 0, 0.85)" }}
            ></div>
          ...
          <div className={styles.legendItem}>
            <div
              className={styles.legendBubble}
              style={{ backgroundColor: "rgba(220, 0, 0, 0.85)" }}
            ></div>
      ...
      <div className={styles.bubblesContainer}>
        ...
        <style>
          {bubbleTokens
            .map((token, index) => generateBubbleKeyframes(index))
            .join("\n")}
        </style>
        ...
        {isLoading ? (
          ...
          <div className={styles.emptyState}>
            ...
            <span className={styles.emptySubtext}>
              Select tokens from the sidebar to visualize
            </span>
          </div>
        ) : (
          bubbleTokens.map((token, index) => {
            ...
            const intensity = Math.min(
              Math.abs(token.priceChange24H || 0) * 3,
              100
            );
            ...
            return (
              <Link
                key={token.assetID}
                to={`/token-details?assetId=${token.assetID}&name=${
                  token.name
                }&logo=${token.logo}&price=${token.latestPrice}&change=${
                  token.priceChange24H
                }&holders=${token.holders}&totalTVL=${
                  token.totalTVL || 0
                }&fullTVL=${token.fullTVL || 0}&stableTVL=${
                  token.stableTVL || false
                }`}
              ...
              >
                ...
                >
                  ...
                  <div
                    className={styles.bubbleChange}
                    style={{
                      fontSize:
                        size > 90 ? "14px" : size > 70 ? "12px" : "10px",
                    }}
                  >
                    {token.priceChange24H > 0 ? "+" : ""}
                    {token.priceChange24H?.toFixed(2)}%
